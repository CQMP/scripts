...
def main(params):
    ...
    nflavors = int(gw_data_in.shape[0]/2)
    ...
    mu = params["mu"]
    iw = fgrid_in*1j
    gw_data = np.zeros((nflavors, gw_data_in.shape[1]), dtype=np.complex128)
    # gw_data = np.zeros((gw_data_in.shape[0]/2, gw_data_in.shape[1]), dtype=np.complex128)
    for x in range(nflavors):
        gw_data[x] = gw_data_in[2*x] + gw_data_in[2*x+1]*1j

    sigma_data = np.zeros(gw_data.shape, dtype=np.complex128)
    vertex_data = np.zeros(np.append([nflavors], vertex_data_in[0].shape), dtype=np.complex128)
    ...
    sigma = np.zeros([nflavors, len(fgrid)], dtype=np.complex128)
    delta = sigma*0
    gw = sigma*0
...
def convert_multidimensional(data):
    ...
    shape_out=shape_out.astype(np.int64) #map(int,shape_out)
...
def read_txt(fname):
    ...
    if use_pandas: 
        ...
        all_data = np.genfromtxt(fname, dtype = float, skip_header = int(has_header), autostrip=True, unpack = True)
    print("shape :", all_data.shape)

    print("Read successfully I guess")
...
def save_grid_object(data,grids,name,h5group):
    ...
    data_is_complex = (data.dtype == np.complex128)
    data = data.astype(np.complex128).view(np.float64).reshape(np.append(data.shape,[2])) if data_is_complex else data 
    ...
    for (i,grid) in zip(range(len(grids)),grids):
        #print i, grid.shape
        grid_is_complex = grids[i].dtype == np.complex128
        grid_g = grids_d.create_group(str(i))
        grid_data = grids[i] if not grid_is_complex else grids[i].astype(np.complex128).view(np.float64).reshape([len(grids[i]),2])
